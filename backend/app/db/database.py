...
Enhanced version of database.py with async-compatible SQLite access.
"""
import asyncio
import json
import os
...
class AsyncMarketDatabase:
    ...
    async def _configure_database(self, conn: aiosqlite.Connection) -> None:
    ...
    async def get_run_summary_page(
        self,
        limit: int = 10,
        offset: int = 0
    ) -> List[Dict[str, Any]]:
        """Get paginated summary rows directly from SQL."""
        await self._ensure_initialized()
    ...
    async def export_to_json(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            # Fetch all set profits in one pass and group by run_id
            cursor = await conn.execute('''
                SELECT run_id, set_slug, set_name, profit_margin, lowest_price
                FROM set_profits
                ORDER BY run_id, set_name
            ''')
            all_profit_rows = await cursor.fetchall()
            profits_by_run: Dict[int, List[Dict[str, Any]]] = {}
            for row in all_profit_rows:
                run_id = row[0]
                profits_by_run.setdefault(run_id, []).append({
                    'set_slug': row[1],
                    'set_name': row[2],
                    'profit_margin': row[3],
                    'lowest_price': row[4]
            ...
            for run in runs:
                run_id = run['run_id']
                set_profits = profits_by_run.get(run_id, [])
                run_data = {
                    'run_info': run,
                    'set_profits': set_profits,
                    'summary': {
